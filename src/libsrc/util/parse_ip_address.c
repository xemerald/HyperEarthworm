...
parse_ip_address_string(const char *str, char hostStr[64], char portStr[10], int *isIPv6)
{
   ...
   if(numeric_address_to_string_ew(*isIPv6, inAddr, inPort, hostStr, portStr) != 0) {
...
/*
    ip_in_same_subnet() accepts three strings specifying valid 
    IPv4 or IPv6 addresses

    "net mask" either represents the concept of a network mask
    in IPv4 or a "prefix" in IPv6 terminology

    if (ip1 & netmask) == (ip2 & netmask), then 1 is returns
    otherwise, 0 is returned, including for all error conditions
...
char ip_in_same_subnet(const char * ip1, const char * ip2, char *netmask)
{
    ...
    //First, verify protocol families for ip1 and ip2:
    if(parse_ip_address_string(ip1, hostStr, portStr, &ip1_is_IPv6) <= 0) {
      //this is an error! probably should warn but for now just return 0:
      return 0;
    }
    if(parse_ip_address_string(ip2, hostStr, portStr, &ip2_is_IPv6) <= 0) {
        return 0;
    }
    if(parse_ip_address_string(netmask, hostStr, portStr, &netmask_is_IPv6) <= 0) {
        return 0;
    }
    //not going to sort out IP family mismatches:
    if(ip1_is_IPv6 != ip2_is_IPv6 != netmask_is_IPv6) {
        return 0;
    }
    //here we know that if ip1 is IPv4, then so is ip2:
    if(ip1_is_IPv6 == 0) {
      ...
      //In IPv6, IPs are 128 bits long
      //  the smallest recommended CIDR subnet is /64
      //  which corresponds to a prefix ("mask") of
      //  ffff:ffff:ffff:ffff:0000:0000:0000:0000
      //  Thus, only the first 64 bits will be checks as a mask
#ifdef _WINNT
      ...
      memset(&(lookupAddr.sin6_addr), 0, sizeof(struct in6_addr));
      if(WSAStringToAddress(TEXT(netmask), AF_INET6, NULL, (LPSOCKADDR)&lookupAddr, (LPINT)lookupLength) != 0) {
         return 0;
      }
      memcpy(&maskAddr, &(lookupAddr.sin6_addr), sizeof(maskAddr));
#else
      ...
      if(inet_pton(AF_INET6, netmask, &maskAddr) != 1) {
         return 0;
      }
#endif
      if(((addr1.s6_addr[0] & maskAddr.s6_addr[0]) == (addr2.s6_addr[0] & maskAddr.s6_addr[0]))
       &&((addr1.s6_addr[1] & maskAddr.s6_addr[1]) == (addr2.s6_addr[1] & maskAddr.s6_addr[1]))
       &&((addr1.s6_addr[2] & maskAddr.s6_addr[2]) == (addr2.s6_addr[2] & maskAddr.s6_addr[2]))
       &&((addr1.s6_addr[3] & maskAddr.s6_addr[3]) == (addr2.s6_addr[3] & maskAddr.s6_addr[3]))
       &&((addr1.s6_addr[4] & maskAddr.s6_addr[4]) == (addr2.s6_addr[4] & maskAddr.s6_addr[4]))
       &&((addr1.s6_addr[5] & maskAddr.s6_addr[5]) == (addr2.s6_addr[5] & maskAddr.s6_addr[5])) 
       &&((addr1.s6_addr[6] & maskAddr.s6_addr[6]) == (addr2.s6_addr[6] & maskAddr.s6_addr[6]))
       &&((addr1.s6_addr[7] & maskAddr.s6_addr[7]) == (addr2.s6_addr[7] & maskAddr.s6_addr[7])))
      {
         return 1;
      } 
   }
